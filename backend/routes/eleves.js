...
router.post('/', async (req, res) => { try { const data = await Eleve.create(req.body); res.status(201).json({ success: true, data }); } catch (e) { res.status(500).json({ success: false, error: e.message }); } });
