...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const qrData = `${req.protocol}://${req.get('host')}/#eleves/${eleveId}`;
        let qrCode = '';
        try { qrCode = await QRCode.toDataURL(qrData); } catch(e) {}
        await conn.query('UPDATE eleves SET qr_code=? WHERE id=?', [qrCode, eleveId]);
        if (responsables && responsables.length) { for (const resp of responsables) { await conn.query('INSERT INTO responsables (eleve_id, nom_complet, lien_parente, telephone, email, whatsapp) VALUES (?,?,?,?,?,?)', [eleveId, resp.nom_complet, resp.lien_parente, resp.telephone, resp.email||null, resp.whatsapp||null]); } }
        await conn.commit();
        res.status(201).json({ success: true, data: { id: eleveId, matricule, qr_code: qrCode } });
...
router.put('/:id', upload.single('photo'), async (req, res) => {
    try {
        ...
        let photoUrl = req.body.photo_existante || null;
        if (req.file) photoUrl = '/uploads/' + req.file.filename;
        const [result] = await pool.query('UPDATE eleves SET nom=?, prenom=?, date_naissance=?, genre=?, adresse=?, photo=? WHERE id=?', [nom, prenom, date_naissance, genre, adresse||null, photoUrl, req.params.id]);
        if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Élève non trouvé' });
        res.json({ success: true, message: 'Élève modifié', photo: photoUrl });
    } catch(e) { res.status(500).json({ success: false, error: e.message }); }
});

router.delete('/:id', async (req, res) => {
    try { const [r]=await pool.query('DELETE FROM eleves WHERE id=?',[req.params.id]); if(r.affectedRows===0) return res.status(404).json({success:false,message:'Élève non trouvé'}); res.json({success:true,message:'Élève supprimé'}); }
    catch(e) { res.status(500).json({success:false,error:e.message}); }
});
...
router.delete('/responsable/:id', async (req, res) => {
    try { const [r]=await pool.query('DELETE FROM responsables WHERE id=?',[req.params.id]); if(r.affectedRows===0) return res.status(404).json({success:false,message:'Responsable non trouvé'}); res.json({success:true,message:'Responsable supprimé'}); }
    catch(e) { res.status(500).json({success:false,error:e.message}); }
});
