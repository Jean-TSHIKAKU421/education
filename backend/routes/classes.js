...
async function copierFeuilleModele(wb, sheetName, nomInstitution, nomClasse) {
    ...
    modeleWs.eachRow({ includeEmpty: true }, (row) => { row.eachCell({ includeEmpty: true }, (cell, colNumber) => { const nc = newWs.getCell(row.number, colNumber); nc.value = cell.value; if (cell.style) { nc.font = { ...cell.font }; nc.fill = { ...cell.fill }; nc.border = { ...cell.border }; nc.alignment = { ...cell.alignment }; nc.numFmt = cell.numFmt; } }); });
...
async function ajouterFeuilleSiAbsente(wb, sheetName, nomInstitution, nomClasse) { if (!wb.getWorksheet(sheetName)) { const ws = await copierFeuilleModele(wb, sheetName, nomInstitution, nomClasse); if (!ws) { const f = wb.addWorksheet(sheetName); f.addRow(['Nom','Prénom','Date de naissance','Genre','Adresse','Classe']); } } }
...
router.post('/institution', async (req, res) => { try { const { nom, niveau } = req.body; const [r] = await pool.query('INSERT INTO institutions (nom, niveau) VALUES (?,?)', [nom, niveau]); const nomInst = nom || 'Complexe Scolaire Avenir'; const wb = await getOrCreateWorkbook(); if (niveau === 'maternelle') { for (const n of ['1ère Maternelle','2ème Maternelle','3ème Maternelle']) { await pool.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,25)', [r.insertId, n, n.split(' ')[0]]); await ajouterFeuilleSiAbsente(wb, n, nomInst, n); } } else if (niveau === 'primaire') { for (let i=1;i<=6;i++) { const nc = `${i}ème Primaire`; await pool.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,35)', [r.insertId, nc, `${i}ème`]); await ajouterFeuilleSiAbsente(wb, nc, nomInst, nc); } } else if (niveau === 'secondaire') { for (const n of ['7ème E.B','8ème E.B']) { await pool.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,40)', [r.insertId, n, n.split(' ')[0]]); await ajouterFeuilleSiAbsente(wb, n, nomInst, n); } } await saveWorkbook(wb); res.json({ success: true, message: 'Institution créée' }); } catch(e) { res.status(500).json({ success: false, error: e.message }); } });

router.post('/institution/secondaire', async (req, res) => { const conn = await pool.getConnection(); try { const { nom, options } = req.body; await conn.beginTransaction(); const [inst] = await conn.query('INSERT INTO institutions (nom, niveau) VALUES (?,?)', [nom, 'secondaire']); const instId = inst.insertId; const wb = await getOrCreateWorkbook(); const nomInst = nom || 'Complexe Scolaire Avenir'; for (const opt of options) { const [optRes] = await conn.query('INSERT INTO options_secondaire (code, nom) VALUES (?,?)', [opt.code, opt.nom]); for (const niv of ['1ère','2ème','3ème','4ème']) { const nc = `${niv} ${opt.nom}`; await conn.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, option_id, capacite) VALUES (?,?,?,?,35)', [instId, nc, niv, optRes.insertId]); await ajouterFeuilleSiAbsente(wb, nc, nomInst, nc); } } for (const n of ['7ème E.B','8ème E.B']) { await conn.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,40)', [instId, n, n.split(' ')[0]]); await ajouterFeuilleSiAbsente(wb, n, nomInst, n); } await saveWorkbook(wb); await conn.commit(); res.json({ success: true, message: 'Secondaire créé' }); } catch(e) { await conn.rollback(); res.status(500).json({ success: false, error: e.message }); } finally { conn.release(); } });
