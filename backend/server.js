const express = require('express');const cors = require('cors');const { requestLogger } = require('./middleware/logger');const { rattraperPresencesManquantes } = require('./middleware/rattrapage');const bodyParser = require('body-parser');const path = require('path');const fs = require('fs');const https = require('https');require('dotenv').config();
...
app.use('/api/auth', require('./routes/auth'));app.use('/api/eleves', require('./routes/eleves'));app.use('/api/classes', require('./routes/classes'));app.use('/api/presences', require('./routes/presences'));app.use('/api/professeurs', require('./routes/professeurs'));app.use('/api/preparations', require('./routes/preparations'));app.use('/api/matieres', require('./routes/matieres'));app.use('/api/titulaires', require('./routes/titulaires'));
...
server.listen(PORT, () => { console.log(`Serveur ${sslOptions?'HTTPS':'HTTP'} démarré sur le port ${PORT}`); setTimeout(() => { rattraperPresencesManquantes(); }, 5000); });
