...
const REGIMES = {
    ANGLAIS: { label: 'Anglais (Lun-Ven)', jours: [1, 2, 3, 4, 5] },
    FRANCAIS: { label: 'Français (Lun-Sam)', jours: [1, 2, 3, 4, 5, 6] }
};

let REGIME_ACTIF = REGIMES.ANGLAIS;
...
function estJourFerie(date = new Date()) {
    const jour = date.getDate(), mois = date.getMonth() + 1, annee = date.getFullYear();
...
function estJourEtude(date = new Date()) {
    const jourSemaine = date.getDay();
    if (jourSemaine === 0) return false;
    if (!REGIME_ACTIF.jours.includes(jourSemaine)) return false;
    if (estJourFerie(date)) return false;
    return true;
}
