class ImportExcelUI {
    ...
    async lancerImport(file) {
        try {
            ...
            const avecStatut = listeFinale.map(e => { const np = String(e.nom || '').toLowerCase().trim(), p = String(e.prenom || '').toLowerCase().trim(); return { ...e, doublon: this.service.existants.some(ex => ex.nom === np && ex.prenom === p) }; });
    ...
    afficherModalOptionsInconnues(optionsInconnues) {
        return new Promise((resolve) => {
            ...
            overlay.innerHTML = `<div class="modal" style="max-width:550px" onclick="event.stopPropagation()"><div class="modal-header"><h3><i class="fas fa-exclamation-triangle" style="color:var(--warning)"></i> Options inconnues</h3><button class="modal-close" id="close-options-inc"><i class="fas fa-times"></i></button></div><div class="modal-body"><p style="color:var(--text-secondary);margin-bottom:1rem">Ces options n'existent pas encore :</p><div class="table-container"><table class="data-table"><thead><tr><th>Option</th><th>Niveau</th><th>Élèves</th></tr></thead><tbody>${lignes}</tbody></table></div><p style="color:var(--text-muted);margin-top:1rem">Voulez-vous créer ces options ?</p></div><div class="modal-footer"><button class="btn btn-ghost" id="btn-ignorer-options-inc"><i class="fas fa-times"></i> Ignorer</button><button class="btn btn-primary" id="btn-creer-options-inc"><i class="fas fa-check"></i> Créer</button></div></div>`;
            document.body.appendChild(overlay);
            document.getElementById('close-options-inc').onclick = () => { overlay.remove(); resolve('cancel'); };
            document.getElementById('btn-ignorer-options-inc').onclick = () => { overlay.remove(); resolve('cancel'); };
    ...
    afficherModalApercu(avecStatut, nouveaux, total) {
        return new Promise((resolve) => {
            ...
            const lignes = avecStatut.slice(0, 100).map((e, i) => {
                const badge = e.doublon ? '<span class="badge badge-warning"><i class="fas fa-copy"></i> Existe déjà</span>' : '<span class="badge badge-success"><i class="fas fa-plus-circle"></i> Nouveau</span>';
                return `<tr style="${e.doublon ? 'opacity:0.5' : ''}"><td>${i + 1}</td><td>${e.nom || ''}</td><td>${e.prenom || ''}</td><td>${String(e.genre || '').toUpperCase().charAt(0)}</td><td>${e.dateNaissance || ''}</td><td>${e.classe || ''}${e.option ? ' - ' + e.option : ''}</td><td>${badge}</td></tr>`;
            }).join('');
            overlay.innerHTML = `<div class="modal" style="max-width:750px;max-height:80vh;display:flex;flex-direction:column" onclick="event.stopPropagation()"><div class="modal-header"><h3><i class="fas fa-list"></i> Aperçu (${total} élèves)</h3><button class="modal-close" id="close-apercu-import"><i class="fas fa-times"></i></button></div><div class="modal-body" style="flex:1;overflow-y:auto;padding-bottom:0.5rem"><p style="color:var(--text-muted);font-size:0.8rem;margin-bottom:1rem"><span class="badge badge-success"><i class="fas fa-plus-circle"></i> ${nouveaux.length} nouveaux</span> <span class="badge badge-warning"><i class="fas fa-copy"></i> ${avecStatut.length - nouveaux.length} déjà existants</span></p><div class="table-container"><table class="data-table"><thead><tr><th>#</th><th>Nom & Postnom</th><th>Prénom</th><th>G</th><th>Date naiss.</th><th>Classe</th><th>Statut</th></tr></thead><tbody>${lignes}</tbody></table></div></div><div class="modal-footer"><button class="btn btn-ghost" id="btn-annuler-import"><i class="fas fa-times"></i> Annuler</button><button class="btn btn-primary" id="btn-lancer-import" ${nouveaux.length === 0 ? 'disabled' : ''}><i class="fas fa-upload"></i> Importer ${nouveaux.length} nouveaux</button></div></div>`;
            document.body.appendChild(overlay);
            document.getElementById('close-apercu-import').onclick = () => { overlay.remove(); resolve(false); };
            document.getElementById('btn-annuler-import').onclick = () => { overlay.remove(); resolve(false); };
    ...
    async executerImport(eleves) {
        ...
        const processNext = (i) => {
            ...
            const e = eleves[i]; const nomPostnom = String(e.nom || '').trim(), prenom = String(e.prenom || '').trim(), date_naissance = String(e.dateNaissance || ''), genre = (String(e.genre || 'M').trim().toUpperCase().charAt(0) === 'F') ? 'F' : 'M', adresse = String(e.adresse || '').trim(), classe_nom = String(e.classe || '').trim(), option_nom = String(e.option || '').trim();
    ...
    ouvrirAlert(titre, message, type) {
        ...
        overlay.innerHTML = `<div class="modal" style="max-width:380px;text-align:center" onclick="event.stopPropagation()"><div class="modal-body" style="padding:2rem 1.5rem"><i class="fas fa-${icones[type] || 'info-circle'}" style="font-size:2.5rem;color:${couleurs[type] || 'var(--primary)'};margin-bottom:0.75rem;display:block"></i><h3 style="margin-bottom:0.5rem">${titre}</h3><p style="color:var(--text-secondary);font-size:0.9rem">${message}</p><button class="btn btn-primary" style="margin-top:1.25rem;width:100%" id="btn-alert-ok"><i class="fas fa-check"></i> OK</button></div></div>`;
        document.body.appendChild(overlay);
