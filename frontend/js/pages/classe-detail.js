...
router.post('/institution', async (req, res) => {
    try {
        const { nom, niveau } = req.body; const [r] = await pool.query('INSERT INTO institutions (nom, niveau) VALUES (?,?)', [nom, niveau]); const nomInst = nom || 'Complexe Scolaire Avenir'; const wb = getOrCreateWorkbook();
        if (niveau === 'maternelle') { for (const n of ['1ère Maternelle','2ème Maternelle','3ème Maternelle']) { await pool.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,25)', [r.insertId, n, n.split(' ')[0]]); ajouterFeuilleSiAbsente(wb, n, nomInst, n); } }
        else if (niveau === 'primaire') { for (let i=1;i<=6;i++) { const nc = `${i}ème Primaire`; await pool.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, capacite) VALUES (?,?,?,35)', [r.insertId, nc, `${i}ème`]); ajouterFeuilleSiAbsente(wb, nc, nomInst, nc); } }
        saveWorkbook(wb); res.json({ success: true, message: 'Institution créée' });
    } catch(e) { res.status(500).json({ success: false, error: e.message }); }
});
...
router.post('/institution/secondaire', async (req, res) => {
    ...
    try {
        const { nom, options } = req.body; await conn.beginTransaction(); const [inst] = await conn.query('INSERT INTO institutions (nom, niveau) VALUES (?,?)', [nom, 'secondaire']); const instId = inst.insertId; const nomInst = nom || 'Complexe Scolaire Avenir'; const wb = getOrCreateWorkbook();
        for (const opt of options) { const [optRes] = await conn.query('INSERT INTO options_secondaire (code, nom) VALUES (?,?)', [opt.code, opt.nom]); for (const niv of ['1ère','2ème','3ème','4ème']) { const nc = `${niv} ${opt.nom}`; await conn.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, option_id, capacite) VALUES (?,?,?,?,35)', [instId, nc, niv, optRes.insertId]); ajouterFeuilleSiAbsente(wb, nc, nomInst, nc); } }
        ...
        saveWorkbook(wb); await conn.commit(); res.json({ success: true, message: 'Secondaire créé' });
...
router.delete('/institution/:id', async (req, res) => {
    const conn = await pool.getConnection();
    try { await conn.beginTransaction(); await conn.query('SET FOREIGN_KEY_CHECKS=0'); await conn.query('DELETE FROM institutions WHERE id=?',[req.params.id]); await conn.query('SET @count=0'); await conn.query('UPDATE institutions SET id=@count:=@count+1 ORDER BY id'); await conn.query('ALTER TABLE institutions AUTO_INCREMENT=1'); await conn.query('SET FOREIGN_KEY_CHECKS=1'); await conn.commit(); res.json({success:true,message:'Niveau supprimé'}); }
    catch(e) { await conn.rollback(); res.status(500).json({success:false,error:e.message}); }
    finally { conn.release(); }
});
...
router.post('/option/secondaire', async (req, res) => {
    const conn = await pool.getConnection();
    try { const { code, nom, institution_id } = req.body; const [optRes] = await conn.query('INSERT INTO options_secondaire (code, nom) VALUES (?,?)', [code, nom]); const nomInst = 'Complexe Scolaire Avenir'; const wb = getOrCreateWorkbook(); for (const niv of ['1ère','2ème','3ème','4ème']) { const nc = `${niv} ${nom}`; await conn.query('INSERT INTO classes (institution_id, nom_classe, niveau_detail, option_id, capacite) VALUES (?,?,?,?,35)', [institution_id, nc, niv, optRes.insertId]); ajouterFeuilleSiAbsente(wb, nc, nomInst, nc); } saveWorkbook(wb); res.json({ success: true, message: 'Option créée' }); }
    catch(e) { res.status(500).json({ success: false, error: e.message }); }
    finally { conn.release(); }
});
