...
CHECK="✓";CROSS="✗";ARROW="→";GEAR="⚙";DATABASE="🗄";PACKAGE="📦";ROCKET="🚀";WARNING="⚠";KEY="🔑";DB="📋";OS_ICON="💻";RELOAD="🔄"
clear
echo ""
echo -e "${BLUE}${BOLD}  ╔════════════════════════════════════════════╗${NC}"
...
echo ""
echo -e "${YELLOW}${RELOAD}  Vérification BrowserSync...${NC}"
if [ ! -d "node_modules/browser-sync" ]; then echo -e "${CYAN}  ${ARROW} Installation BrowserSync...${NC}";npm install -D browser-sync 2>&1 | while IFS= read -r line; do echo -e "  $line"; done;echo -e "${GREEN}  ${CHECK} BrowserSync installé${NC}";else echo -e "${GREEN}  ${CHECK} BrowserSync présent${NC}";fi
echo ""
echo -e "${YELLOW}${ROCKET}  Démarrage du serveur...${NC}"
echo ""
LOCAL_IP=$(hostname -I 2>/dev/null | awk '{print $1}');[ -z "$LOCAL_IP" ] && LOCAL_IP="localhost"
PORT=3443
nodemon server.js > /tmp/nodemon-output.txt 2>&1 &
sleep 3
npx browser-sync start --proxy https://localhost:${PORT} --files "../frontend/**/*" --https --no-open > /tmp/browsersync-output.txt 2>&1 &
sleep 2
echo -e "${GREEN}${BOLD}  ╔════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}${BOLD}  ║${NC}  ${WHITE}${BOLD}✅ Serveur démarré avec succès !${NC}${GREEN}${BOLD}       ║${NC}"
echo -e "${GREEN}${BOLD}  ╚════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${CYAN}  🌐 Application   : ${WHITE}https://localhost:${PORT}${NC}"
echo -e "${CYAN}  📡 Réseau        : ${WHITE}https://${LOCAL_IP}:${PORT}${NC}"
echo -e "${CYAN}  🔄 Live Reload   : ${WHITE}https://localhost:3000${NC}"
