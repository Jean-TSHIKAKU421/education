...
if [ "$LOCAL" = "$REMOTE_HASH" ]; then
    ...
    echo "";echo -e "${GREEN}${SUCCESS}  Aucune mise à jour disponible${NC}";echo "";exit 0
fi
...
echo ""
git pull origin "$BRANCH" > /tmp/git-output.txt 2>&1
PULL_EXIT=$?
while IFS= read -r line; do
    [ -z "$line" ] && continue
    [[ $line == *"Updating"* ]] && { echo -e "${GREEN}  ${CHECK} $line${NC}";continue; }
    [[ $line == *"Fast-forward"* ]] && { echo -e "${GREEN}  ${CHECK} $line${NC}";continue; }
    [[ $line == *"Already up to date"* ]] && { echo -e "${GREEN}  ${CHECK} Déjà à jour${NC}";continue; }
    [[ $line == *"Déjà à jour"* ]] && { echo -e "${GREEN}  ${CHECK} Déjà à jour${NC}";continue; }
